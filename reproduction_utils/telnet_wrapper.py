...
def parse_telnet_args(parser: ArgumentParser):
    parser.add_argument("--telnetIp", help="the id of localhost", default="localhost")
...
class TelnetWrapper():
    ...
    def load_snapshot(self, snap_shot_name):
        ...
        if "OK" not in t.decode():
            raise Exception("Timeout when loading snapshot. The snapshot may be lost or damaged")
        time.sleep(3)
